...
final class UserRepository implements UserRepositoryInterface
{
    ...
    {
        $row = $this->db->fetchOne("SELECT * FROM {$this->table} WHERE username = ? OR email = ?", [$login, $login]);
        if (!$row) return null;
        return ['user' => new User($row), 'hash' => $row['password_hash']];
    }
    ...
    public function create(User $user, string $plainPassword, int $referralId = 0): int
    {
        ...
        $sql = "INSERT INTO {$this->table}
                (username, password_hash, email, `group`, money, all_money, avatar, reg_data, reg_ip, referral, banned, created_at, updated_at, count_theard, count_post, count_like)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $params = [
            $user->getUsername(), $hashed, $user->getEmail(), $user->getGroup(),
            $user->getMoney(), $user->getAllMoney(), $user->getAvatar(),
            $user->getRegData(), $user->getRegIp(), $referralId,
            (int)$user->isBanned(), $now, $now,
            $user->getCountTheard(), $user->getCountPost(), $user->getCountLike()
        ];
    ...
    public function update(User $user): void
    {
        $sql = "UPDATE {$this->table} SET
                username = ?, email = ?, `group` = ?, money = ?, all_money = ?, avatar = ?,
                banned = ?, remember_token = ?, updated_at = ?, count_theard = ?, count_post = ?, count_like = ?
                WHERE id = ?";
        $params = [
            $user->getUsername(), $user->getEmail(), $user->getGroup(),
            $user->getMoney(), $user->getAllMoney(), $user->getAvatar(),
            (int)$user->isBanned(), $user->getRememberToken(), time(),
            $user->getCountTheard(), $user->getCountPost(), $user->getCountLike(),
            $user->getId()
        ];
        $this->db->query($sql, $params);
    ...
    {
        $hashed = password_hash($plainPassword, PASSWORD_DEFAULT);
        $this->db->query("UPDATE {$this->table} SET password_hash = ?, updated_at = ? WHERE id = ?", [$hashed, time(), $userId]);
    }
    ...
    public function findAllPaginated(int $page, int $perPage): array
    {
        ...
        $rows = $this->db->fetchAll("SELECT * FROM {$this->table} ORDER BY id DESC LIMIT ? OFFSET ?", [$perPage, $offset]);
    ...
    {
        $like = "%{$query}%";
        $row = $this->db->fetchOne("SELECT COUNT(*) as cnt FROM {$this->table} WHERE username LIKE ? OR email LIKE ?", [$like, $like]);
        return (int)($row['cnt'] ?? 0);
    }

    public function getReferrals(int $userId): array
    {
        return $this->db->fetchAll("SELECT id, username, email, reg_data FROM {$this->table} WHERE referral = ? ORDER BY id DESC", [$userId]);
    }

    public function getReferralEarnings(int $userId): int
    {
        $row = $this->db->fetchOne("SELECT referral_earnings FROM {$this->table} WHERE id = ?", [$userId]);
        return $row ? (int)$row['referral_earnings'] : 0;
    }

    public function addReferralEarnings(int $userId, int $amount): void
    {
        $this->db->query("UPDATE {$this->table} SET referral_earnings = referral_earnings + ? WHERE id = ?", [$amount, $userId]);
    }

    public function updateReferral(int $userId, int $referralId): void
    {
        $this->db->query("UPDATE {$this->table} SET referral = ? WHERE id = ?", [$referralId, $userId]);
    }
    ...
    public function getRegistrationsLastDays(int $days): array
    {
        ...
        for ($i = $days - 1; $i >= 0; $i--) {
            ...
            $end = $start + 86400;
            $stmt = $this->db->query("SELECT COUNT(*) as cnt FROM {$this->table} WHERE created_at >= ? AND created_at < ?", [$start, $end]);
            $row = $stmt->fetch();
